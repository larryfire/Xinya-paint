  ...
  if (isMobile) {
    return (
      <>
        ...
        {/* 底部抽屉 */}
        <div
          className={`absolute bottom-0 left-0 right-0 z-50 bg-slate-900/98 backdrop-blur-md border-t border-slate-700/50 rounded-t-2xl shadow-2xl shadow-black/50 transition-all duration-300 ease-out overflow-y-auto ${
            mobileExpanded ? "max-h-[85vh]" : "max-h-[55vh]"
          }`}
        >
          {/* 拖拽指示条 */}
          <div className="sticky top-0 z-10 bg-slate-900/98 pt-2 pb-1 flex flex-col items-center">
            <button
              className="w-10 h-1.5 bg-slate-600 rounded-full mb-1"
              onClick={() => setMobileExpanded(!mobileExpanded)}
              aria-label={mobileExpanded ? "收起详情" : "展开详情"}
            />
            <button
              className="absolute top-2 right-3 w-7 h-7 flex items-center justify-center rounded-full bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white transition-colors"
              onClick={handleClose}
            >
              <X className="h-4 w-4" />
            </button>
          ...
          <div className="px-4 pb-6 space-y-4">
            {selectedShip && <ShipDetailContent />}
            {selectedDock && !selectedShip && <DockDetailContent />}
          </div>
          ...
          {useSceneStore.getState().attendanceOpen && (
            <AttendanceForm
              attTeamId={attTeamId}
              attWorkers={attWorkers}
              submitting={submitting}
              onTeamIdChange={setAttTeamId}
              onWorkersChange={setAttWorkers}
    ...
    return (
      <>
        ...
        <div>
          ...
          {(
            (selectedDock as unknown as Record<string, unknown>)
              .activeAttendance as unknown[] ?? []
          ).length === 0 ? (
            <p className="text-xs text-slate-600 text-center py-2">
              暂无活跃出勤
            </p>
  ...
  return (
    <div className="bg-slate-900 border-t border-slate-700/50 rounded-t-xl p-4 w-full space-y-3">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-slate-100">开始出勤</h4>
        <button
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() => onWorkersChange(Math.max(1, attWorkers - 1))}
          >
          ...
          <button
            className="w-8 h-8 flex items-center justify-center bg-slate-800 border border-slate-700 rounded-lg text-slate-200 hover:bg-slate-700"
            onClick={() => onWorkersChange(attWorkers + 1)}
          >
            <Plus className="h-3 w-3" />
          </button>
        </div>
...
function AttendanceRow({
  ...
  return (
    <div className="flex items-center justify-between bg-slate-800/50 rounded-lg px-3 py-2">
      ...
      <button
        className="w-6 h-6 flex items-center justify-center rounded-full bg-red-500/20 text-red-400 hover:bg-red-500/40 transition-colors ml-2 shrink-0"
        onClick={onEnd}
      >
        <Minus className="h-3 w-3" />
      </button>
